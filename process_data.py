...
import pandas as pd
# import os to read directory contents
import os
...
def featurize_data(mat_prop):
    ...
    X_file = feature_dir+'X_'+mat_prop+'.csv'
    y_file = feature_dir+'y_'+mat_prop+'.csv'
    f_file = feature_dir+'formula_'+mat_prop+'.csv'

    X.to_csv(X_file, index=False)
    y.to_csv(y_file, index=False, header=True)
    formula.to_csv(f_file, index=False, header=True)


def get_split(mat_prop, seed_num=1):
    featurized = os.listdir(feature_dir)
    X_file, y_file = 'X_'+mat_prop+'.csv', 'y_'+mat_prop+'.csv'
    if X_file in featurized and y_file in featurized:
        print('Reading featurized csv for: '+mat_prop)
        X = pd.read_csv(feature_dir+'X_'+mat_prop+'.csv')
        y = pd.read_csv(feature_dir+'y_'+mat_prop+'.csv')
        formula = pd.read_csv(feature_dir+'formula_'+mat_prop+'.csv')
    else:
        print(mat_prop+' not found: featurization initialized')
        featurize_data(mat_prop)
        X = pd.read_csv(feature_dir+'X_'+mat_prop+'.csv')
        y = pd.read_csv(feature_dir+'y_'+mat_prop+'.csv')
        formula = pd.read_csv(feature_dir+'formula_'+mat_prop+'.csv')
...
if __name__ == '__main__':
    generate_split(mat_props, seed_num=1)
