...
from composition import _fractional_composition as frac_comp
...
def read_in_elpasolite():
    ...
    X_elp, y_elp, formula_elp = generate_features(df_elp, reset_index=True)
    return X_elp, y_elp, formula_elp
...
def read_in_pcd():
    ...
    X_pcd, y_pcd, formula_pcd = generate_features(df_pcd, reset_index=True)
    return X_pcd, y_pcd, formula_pcd
...
def get_model(X, y, formula):
    mat_prop = 'ael_bulk_modulus_vrh'
    data = get_split(mat_prop, seed_num=1)
    ...
    percent = 0.01
    for value in y_all:
        if value > y_all.iloc[-int(y_all.shape[0]*percent)]:
...
def get_prediction(lr, formula_all, formula, threshold=0.5, top_N=None):
...
X_pcd, y_pcd, formula_pcd = read_in_pcd()
...
in_train, out_of_train = get_prediction(lr, formula_all, formula, threshold=0.5)

if len(in_train + out_of_train) == 0:
    print('\n**************************************************************' +
          '\nthere are not compounds with a probability above the threshold' +
          '\n**************************************************************')
    ...
    ax1.bar(v1.index, v1/sum(v1), edgecolor='k', color='#abdda6', label='Original 1%')
    for x, y, s in zip(v1.index, v1/sum(v1), v1.astype(str).tolist()):
        ax1.text(x, y+0.013, s, horizontalalignment='center', verticalalignment='center')
    ax2.bar(v2.index, v2/sum(v2), edgecolor='k', color='#fdae65', label='PCD Screened')
    for x, y, s in zip(v2.index, v2/sum(v2), v2.astype(str).tolist()):
        ax2.text(x, y+0.013, s, horizontalalignment='center', verticalalignment='center')
    ...
    ax1.legend()
    ax2.legend()
plt.savefig('figures/PCD_predictions.png', dpi=300, bbox_inches='tight')


# %%


X_elpasolite, y_elpasolite, formula_elpasolite = read_in_elpasolite()
...
if len(in_train + out_of_train) == 0:
    ...
    ax1.legend()
    ax2.legend()
    plt.savefig('figures/Elpasolites_predictions.png', dpi=300, bbox_inches='tight')
