...
def get_performance(mat_props, seed):
    ...
    #    best_params_rr = {'alpha': 0.0021544346900318843}
        ...
        metrics[mat_prop] = {'precision': [], 'recall': [], 'f1': []}
        threshold = optimize_threshold(y_train_labeled, y_train_predicted_rr)
        ...
        threshold = 0.5
        y_pred_svc = [1 if x >= threshold else 0 for x in y_probability_test_svc]
        ...
        for prediction in predictions:
            ...
            precision, recall, f1, support = \
                        precision_recall_fscore_support(y_test_labeled, prediction)
            if precision[1] == 0:
                if precision == 10:
                    pass
            metrics[mat_prop]['precision'].append(precision[1])
            metrics[mat_prop]['recall'].append(recall[1])
            metrics[mat_prop]['f1'].append(f1[1])
...
#    colors = plt.rcParams['axes.prop_cycle'].by_key()['color']
    ...
    for df_mean, df_std, ax in zip(means, stds, axes):
        ...
        ax.plot(prop_loc, df_mean.loc['rr'], '-x', color=colors[0], linewidth=2, label='Ridge')
        ax.plot(prop_loc, df_mean.loc['svr'], '-s', color=colors[3], linewidth=2, label='SVR')
        ax.plot(prop_loc, df_mean.loc['lr'], '--d', color=colors[2], linewidth=3, label='Logistic')
        ax.plot(prop_loc, df_mean.loc['svc'], '--*', color=colors[1], linewidth=3, label='SVC')
    ...
    plt.xticks(np.array(prop_loc), labels=[prop.title() for prop in all_props], rotation=30, ha='center', rotation_mode='anchor')
